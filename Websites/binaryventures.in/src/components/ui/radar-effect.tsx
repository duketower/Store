...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-[#E5097F] to-transparent" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(229, 9, 127, ${0.18 - idx * 0.018})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[rgba(229,9,127,0.25)] bg-[rgba(15,15,15,0.8)] shadow-inner backdrop-blur-sm">
        {icon}
      </div>
      <div className="hidden rounded-md px-2 py-1 md:block">
        <div className="text-center text-xs font-medium text-[rgba(255,255,255,0.5)]">
          {text}
        </div>
      </div>
    </motion.div>
